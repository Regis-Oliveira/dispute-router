...
// Assist's own tests, separated from assist_test.go by what they need rather
// than by what they are about: every one of them runs the flow end to end, and
// the flow writes agent_runs, so they need the scratch database scratchDB
// creates and drops. That file's integration tag has to come with them. The one
// assist test that reaches no database - the prompt fingerprint - stays
// untagged in assist_test.go.
