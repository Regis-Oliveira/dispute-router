...
func testLoop(t *testing.T, script *llmtest.ScriptedCompleter, budget Budget) *Loop {
	t.Helper()
	if budget.MaxTokens == 0 {
		budget.MaxTokens = 2048
	}
	return NewLoop(script, offlineRegistry(), "test-model", llm.Pricing{
...
func badCall(id string) llm.ToolUse {
	return llm.ToolUse{ID: id, Name: "list_disputes", Input: json.RawMessage(`{"nope":1}`)}
}
